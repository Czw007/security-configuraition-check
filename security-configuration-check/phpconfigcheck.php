...
if (php_sapi_name() == "cli") {
	...
	if (function_exists('getopt')) {
		foreach (getopt("hja") as $k => $v) { //getopt — 从命令行参数列表中获取选项
			switch ($k) {
				case 'h':
	...
	// mtime check
	$cfg['PCC_DISABLE_MTIME'] = getenv("PCC_DISABLE_MTIME");//getenv — 获取一个环境变量的值
	$cfg['SCRIPT_FILENAME'] = getenv("SCRIPT_FILENAME");
	if (!$cfg['PCC_DISABLE_MTIME'] && $cfg['SCRIPT_FILENAME'] !== FALSE) {
		if (filemtime($cfg['SCRIPT_FILENAME']) + 2*24*3600 < time()) {//脚本在非 CLI 环境中只能工作两天
	...
	// IP check
	$cfg['REMOTE_ADDR'] = getenv("REMOTE_ADDR");//getenv — 获取一个环境变量的值
...
//上面的内容都是在控制输出格式和一些运行环境的设置，不是主要功能，可以忽略
// functions
